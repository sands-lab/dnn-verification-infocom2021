...
def enum_solutions(ilpsolver, block_vars_names, vars_print, vars_return, stop_cond=None, verbose=True):
    sol_values_total = list()
    sol_values_return = list()
    while(True):
        ...
        for s in range(nb_solutions):
            ...
            r_vars, r_values = get_solution(ilpsolver, solutions, vars_return, sol_id=s)
            sol_values_return.append(r_values)

            if stop_cond is not None and stop_cond(vars, values):
                return sol_values_total, sol_values_return
